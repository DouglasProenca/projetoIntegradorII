...
import com.sistema.desktop_cr7_imports.objects.PropertiesSystem;
			...
			if (String.valueOf(txtSenha.getPassword()).equals("1234")) {
				...
				if (fc.showSaveDialog(this) == JFileChooser.FILES_ONLY) {
					...
					try {
						...
						PreparedStatement instrucaoSQL = conexao.prepareStatement("BACKUP DATABASE [" + ps.getDatabase()
								+ "] TO  DISK = '" + arquivo + "' WITH FORMAT NAME = 'Backup'");
						instrucaoSQL.executeUpdate();
						progbarProgresso.setMaximum(2);
						progbarProgresso.setValue(100);
					} catch (SQLException ex) {
						JOptionPane.showMessageDialog(this, ex.getMessage(), "Aviso de Falha",
